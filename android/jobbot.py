...
def build_flask_app():
    ...
    <head>
        ...
        <style>
            :root {
                ...
                --text: #101828; --text-dim: #667085;
                --ok-bg: #ecfdf3; --ok-text: #067647;
                --warn-bg: #fffaeb; --warn-text: #b54708;
                --danger-bg: #fef3f2; --danger-text: #b42318;
                --info-bg: #eff4ff; --info-text: #1d4ed8;
            }
            * { box-sizing: border-box; }
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; margin: 0; padding: 20px; background: var(--bg); color: var(--text); min-height: 100vh; }
            .topbar { display:flex; justify-content:space-between; align-items:center; margin-bottom: 18px; }
            h1 { color: var(--text); font-weight: 600; font-size: 19px; margin: 0; }
            h3 { color: var(--text); font-weight: 600; font-size: 15px; margin: 0 0 4px; }
            a { color: var(--accent); text-decoration: none; }
            .whoami { font-size: 13px; color: var(--text-dim); }
            .whoami a { color: var(--text-dim); text-decoration: underline; margin-left: 6px; }
            .card { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 16px; margin-bottom: 12px; }
            .btn { background: var(--accent); color: white; padding: 9px 16px; border: none; border-radius: 8px; text-decoration: none; cursor: pointer; display:inline-block; margin: 3px 6px 3px 0; font-size: 13px; font-weight: 600; }
            .btn:hover { background: var(--accent-dark); }
            .btn-danger { background: var(--danger-text); }
            .btn-outline { background: transparent; color: var(--text); border: 1px solid var(--border); }
            .btn-outline:hover { background: #f2f4f7; }
            .badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; background: #f2f4f7; color: var(--text-dim); margin: 2px 6px 2px 0; }
            .badge-danger { background: var(--danger-bg); color: var(--danger-text); }
            .badge-ok { background: var(--ok-bg); color: var(--ok-text); }
            .badge-info { background: var(--info-bg); color: var(--info-text); }
            .badge-warn { background: var(--warn-bg); color: var(--warn-text); }
            .note { background: var(--card); border: 1px solid var(--border); padding: 12px 16px; border-radius: 12px; font-size: 13px; color: var(--text-dim); line-height: 1.6; }
            textarea { width: 100%; min-height: 340px; font-family: ui-monospace, Menlo, Consolas, monospace; font-size: 13px; padding: 10px; background: var(--bg); color: var(--text); border: 1px solid var(--border); border-radius: 8px; }
            input[type=text] { width: 100%; padding: 9px; font-size: 14px; background: var(--card); color: var(--text); border: 1px solid var(--border); border-radius: 8px; }
            code { background: var(--accent-soft); color: var(--accent-dark); padding: 1px 5px; border-radius: 4px; }
        </style>
    </head>
    <body>
        <div class="topbar">
            <h1>💙 JobBot</h1>
            <span class="whoami">{{ profile_name }}<a href="/logout">switch</a></span>
        </div>
        <p class="note">
        ...
        <p style="margin-top:14px;">
            ...
            <a href="/own-cv" class="btn btn-outline">Upload Own CV</a>
        </p>
        <div>
            {% for job in jobs %}
            <div class="card">
                <h3>{{ job.title }} <span style="font-weight:400;color:var(--text-dim);">— {{ job.company or 'Unknown' }}</span></h3>
                <p style="color:var(--text-dim);font-size:13px;"><strong style="color:var(--text);">Location:</strong> {{ job.location }} &nbsp;|&nbsp; <strong style="color:var(--text);">Contact:</strong> {{ job.contact_email or 'N/A' }}</p>
                <p>
                    <span class="badge badge-ok">Match: {{ job.match_score }}%</span>
                    <span class="badge badge-danger">Scam risk: {{ job.scam_score }}%</span>
                    <span class="badge">Status: {{ job.status }}</span>
                    {% if job.cv_mode == 'custom' %}
                        <span class="badge badge-info">CV: custom ATS CV</span>
                    {% elif job.cv_mode == 'own' %}
                        <span class="badge badge-info">CV: own CV</span>
                    {% else %}
                        <span class="badge badge-warn">CV: not chosen yet</span>
                    {% endif %}
                    {% if job.email_edited %}<span class="badge badge-info">Email: edited by you</span>
                    {% elif job.email_body %}<span class="badge">Email: auto-draft</span>
                    {% else %}<span class="badge badge-warn">Email: none yet</span>{% endif %}
                </p>
                <p style="font-size:13px;"><strong>Choose the CV for this job:</strong><br>
                ...
                <p>
                    <a href="/email/{{ job.uid }}" class="btn btn-outline">View / Edit Email</a>
                    <a href="/regen/{{ job.uid }}" class="btn btn-outline">Regenerate Email</a>
    ...
    <head>
        ...
        <style>
            :root {
                --bg: #f7f8fa; --card: #ffffff; --border: #e4e7ec;
                --accent: #2563eb; --accent-dark: #1d4ed8; --text: #101828; --text-dim: #667085;
            }
            * { box-sizing: border-box; }
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; margin: 0; padding: 20px; background: var(--bg); color: var(--text); min-height: 100vh; }
            .card { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 20px; }
            h2 { font-size: 17px; font-weight: 600; margin: 0 0 4px; }
            .btn { background: var(--accent); color: white; padding: 9px 16px; border: none; border-radius: 8px; text-decoration: none; cursor: pointer; display:inline-block; font-size: 13px; font-weight: 600; }
            .btn:hover { background: var(--accent-dark); }
            .btn-outline { background: transparent; color: var(--text); border: 1px solid var(--border); }
            .btn-outline:hover { background: #f2f4f7; }
            textarea { width: 100%; min-height: 380px; font-family: ui-monospace, Menlo, Consolas, monospace; font-size: 13px; padding: 10px; background: var(--bg); color: var(--text); border: 1px solid var(--border); border-radius: 8px; }
            input[type=text] { width: 100%; padding: 9px; font-size: 14px; background: var(--card); color: var(--text); border: 1px solid var(--border); border-radius: 8px; }
            label { font-weight: 600; font-size: 13px; display:block; margin-top: 14px; color: var(--text-dim); }
            small { color: var(--text-dim); }
        </style>
    </head>
    <body>
        <div class="card">
            <h2>{{ job.title }} <small style="font-weight:400;">— {{ job.company or 'Unknown' }}</small></h2>
            <p style="color:var(--text-dim);font-size:13px;line-height:1.7;"><strong style="color:var(--text);">To:</strong> {{ job.contact_email or '(no contact email found)' }}<br>
            ...
            <form method="post">
                ...
                <p>
                    ...
                    <a href="/regen/{{ job.uid }}" class="btn btn-outline">Discard &amp; Regenerate</a>
    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            h2 { font-size: 16px; font-weight: 600; margin: 6px 0 4px; }
            h3 { font-size: 14px; font-weight: 600; margin: 6px 0 4px; }
            .btn { background: var(--accent); color: white; padding: 9px 16px; border: none; border-radius: 8px; text-decoration: none; cursor: pointer; display:inline-block; font-size: 13px; font-weight: 600; }
            .btn:hover { background: var(--accent-dark); }
            .btn-outline { background: transparent; color: var(--text); border: 1px solid var(--border); }
            .btn-outline:hover { background: #f2f4f7; }
            input[type=file] { color: var(--text-dim); }
            .tag { display:inline-block; font-size: 11px; font-weight: 700; letter-spacing: 0.04em; text-transform: uppercase; background: var(--info-bg); color: var(--info-text); padding: 2px 8px; border-radius: 6px; }
            .tag-alt { background: #f2f4f7; color: var(--text-dim); }
        </style>
    </head>
    <body>
        <div class="card">
            <span class="tag">Recommended on phone</span>
            <h2>Import from Downloads</h2>
            <p style="color:var(--text-dim);font-size:13px;">Current file: <strong style="color:var(--text);">{{ own_cv or 'none uploaded yet' }}</strong></p>
            <p style="font-size:13px;">{{ message }}</p>
            <p style="color:var(--text-dim);font-size:13px;">Copy or save your CV file into your phone's <strong style="color:var(--text);">Downloads</strong> folder first (e.g. from WhatsApp, email, or Google Drive), then tap below to pick it from a list.</p>
            <p><a href="/own-cv/browse" class="btn">Browse Downloads folder</a>
               <a href="/" class="btn btn-outline">Back to Dashboard</a></p>
        </div>
        <div class="card">
            <span class="tag tag-alt">Desktop / Termux only</span>
            <h3>Upload via file picker</h3>
            <p style="color:var(--text-dim); font-size:13px;">This button opens your device's native file picker. It works in a regular browser, but does not work inside the packaged Android app.</p>
            <form method="post" enctype="multipart/form-data">
                <input type="file" name="cvfile" accept=".pdf,.docx,.doc,.txt,.rtf,.odt">
                <p><button type="submit" class="btn btn-outline">Upload</button></p>
            </form>
        </div>
        <p style="font-size:12px;color:var(--text-dim);">Any job set to <strong style="color:var(--text);">own CV</strong> gets this exact file attached, untouched.
    ...
    OWN_CV_BROWSE_TEMPLATE = """
    <!DOCTYPE html>
    <html>
    <head>
        <title>Choose your CV</title>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <style>
            :root {
                --bg: #f7f8fa; --card: #ffffff; --border: #e4e7ec;
            ...
            * { box-sizing: border-box; }
            ...
            .card { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 14px 16px; margin-bottom: 8px; display:flex; justify-content:space-between; align-items:center; gap:10px; }
            .filename { font-size: 13px; word-break: break-all; }
            .btn { background: var(--accent); color: white; padding: 8px 14px; border: none; border-radius: 8px; text-decoration: none; cursor: pointer; font-size: 13px; font-weight: 600; white-space: nowrap; }
            .btn:hover { background: var(--accent-dark); }
            .btn-outline { background: transparent; color: var(--text); border: 1px solid var(--border); }
            .btn-outline:hover { background: #f2f4f7; }
    ...
    <body>
        ...
        {% if files %}
            {% for f in files %}
            <div class="card">
                <span class="filename">{{ f.split('/')[-1] }}</span>
                <form method="post" action="/own-cv/import">
            ...
            {% endfor %}
        {% else %}
            <p class="empty">No CV files found in: {{ scan_dirs|join(', ') }}</p>
            <p class="empty">Save or copy your CV (PDF, Word, or text file) into your phone's Downloads folder, then come back and tap "Browse Downloads folder" again.</p>
        {% endif %}
        <p><a href="/own-cv" class="btn btn-grey">Back</a></p>
    </body>
    </html>
    """
    ...
    # The Android WebView shell used by the packaged app cannot open the
    # native file picker (there's no code wired up to relay the browser's
    # file-chooser event to an Android activity). As a workaround, this
    # lets the user copy their CV into their phone's Downloads folder,
    # then pick it from a plain list rendered by JobBot itself, using
    # ordinary filesystem access instead of the native picker.
    ANDROID_SCAN_DIRS = [
    ...
    @app.route("/own-cv/browse")
    def own_cv_browse():
        found = []
        for d in ANDROID_SCAN_DIRS:
            if os.path.isdir(d):
        ...
        return render_template_string(OWN_CV_BROWSE_TEMPLATE, files=[str(f[0]) for f in found], scan_dirs=ANDROID_SCAN_DIRS)

    @app.route("/own-cv/import", methods=["POST"])
    def own_cv_import():
        chosen = request.form.get("path", "")
        message = ""
        if chosen and os.path.isfile(chosen):
            try:
                dest = store_own_cv(chosen)
                message = f"Imported and set as your own CV: {dest.name}"
            except Exception as e:
                message = f"Could not import that file: {e}"
        else:
            message = "That file could not be found. Make sure it's still in the folder and try again."
        own = get_own_cv_path()
        return render_template_string(OWN_CV_TEMPLATE, own_cv=str(own) if own else "", message=message)
